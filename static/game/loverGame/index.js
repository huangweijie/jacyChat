...
function perMove(event) {
	...
	requestAnimationFrame(perMove(event))
}
...
window.addEventListener("deviceorientation", function(event) {
	...
	// if (new Date() - timeControl >= 50) {
	// if (timeControl == 1) {
	// moveId = requestAnimationFrame(perMove(event));
	// timeControl = 2;
	// }
...
function addListener(element, e, fn) {
	if (element.addEventListener) {
		...
		// console.log('eventAdd1');
	} else {
		element.attachEvent("on" + e, fn);
		// console.log('eventAdd2');
	}
}
...
function stop() {
	...
	$('.addWeight').text(gameData.cutSpeed * 10);
...
function game() {
	...
	ctx.fillText(gameData.points, canvasW - 55, 17 + pointsHeight * B)
		// }
		// playid = requestAnimationFrame(playing);
		// console.log(timer);
	var nowTime = new Date();
	// console.log(new Date())
	// while (nowTime - timer < 50) {
	// 	nowTime = new Date();
	// }
